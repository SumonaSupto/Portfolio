...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    try {
      const firebaseUrl = `https://portfoliochatbot-35fdc-default-rtdb.firebaseio.com/api_data/${encodeURIComponent(
        messageText
      )}.json`;
      ...
      if (firebaseData && firebaseData.answer) {
        ...
        const apiResponse = await fetch(
          "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=AIzaSyD85ugaPF-NxBtpIl_7N5BzZ4uKOWxXw0U",
          {
            method: "POST",
            headers: {
        ...
        if (botReply) {
          ...
          const errorMessage =
            "Sorry, I couldn't understand that. (Unexpected response structure)";
      ...
      const errorMessage =
        "Error: Unable to contact the API or process the response.";
      addMessage(errorMessage, "bot-message");
      speak(errorMessage);
    }
  });
